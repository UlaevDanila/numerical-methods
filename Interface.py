    ...
    def btnclick(self):
        ...
        try:
            if (a is not None) or (b is not None) or (epsilon is not None) or (y0 is not None):
                ...
                n = 0
                for i in x:
                    self.table.setItem(n, 0, QTableWidgetItem(str(i)))
                    print(i)
                    n += 1
                n = 0
                for i in y:
                    self.table.setItem(n, 1, QTableWidgetItem(str(i)))
                    n += 1
                n = 0
                for i in y1:
                    self.table.setItem(n, 2, QTableWidgetItem(str(i)))
                    n += 1
                n = 0
                for i in delta:
                    self.table.setItem(n, 3, QTableWidgetItem(str(i)))
                    n += 1
